...
def insert_data(connection, df, batch_size=1000):
    ...
    INSERT IGNORE INTO commercial (
        Id_Commercial, Nom_Commercial, ID_Chef_de_Marche, Statut_Commercial, Jour_par_an, Weekends_par_an, Jour_Conges, RTT, Recuperation, Jour_Feries, Autres_jours_non_travailles, Administration_Commercial, Supervision, Convention, TA_Hors_Secteur, Event_Salon, Formations, Autres_jours_mission_hors_visite_Commercial, Solde_jours_de_travail_par_an, Solde_jours_statiques_par_an, Nombre_de_visite_client_par_jour, `Solde_visite_terrain_client_par_an(Possibles)`, Nombre_de_visite_prospects_par_jour, Pourcentage_des_visites_prospects, `Solde_visite_terrain_prospects_par_an(Possibles)`, `Solde_visite_terrain_client_par_an(Necessaire)`, `Solde_visite_terrain_client_par_an_necessaire(simulee)`, `Nombres_de_visite_Commercial_par_an_(Possibles)`
    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """
    commercial_data = [
        (row['Code Commercial'], row['Nom Commercial'], row['Code Chef de marché'], 0, 365, 104, 25.0, 10.0, 0.0, 9.0, 0.0, 43.39, 5.43, 8.13, 2.71, 5.43, 0.01, 0.0, 217.0, 65.105, 5, 759, 1.25, 20.02, 190.0, 0, 0, 949.0)
    ...
    INSERT IGNORE INTO client (
        ID_Client, Code_Client, Nom_Client, Region_Client, Code_Postal, Ville_Client, Pays_Client, Circuit_Client, Activite_Client, Potentiel_Client, Fidelisation_Client, CA_Client, Couche, Regle_Prod, Nombre_de_visites_par_an
    ...
    """
    visite_data = [
        (replace_nan(row['Code Visite']), replace_nan(row['Code client']), replace_nan(row['Code Commercial']), replace_nan(row.get('Date Visite')), replace_nan(row.get('Type Visite')))
        for _, row in df.iterrows()
